...
const theNumbers = document.querySelector(`.theNumbers`);
...
function changeNumsAndOperatorInDOM(){  //Replace the numbers in the DOM with new random numbers
    ...
    function rmXtraNums(){
        ...
        if(i>numOfRows){
            let xtraNum = document.getElementById(`num${i}`);
    ...
    function changeNumsAndAddXtraNumsIfNeeded(){
        for(let i=1; i<=numOfRows; i++){
          if (document.getElementById(`num${i}`) != null){   //If the element with the id exists...
            let numb = document.getElementById(`num${i}`);
...
function handleTimer(){
    ...
    timeInterval = setInterval(function(){
        ...
        let min = Math.floor((dTotalSec % 3600)/60);
        let sec = (dTotalSec % 3600)%60;
